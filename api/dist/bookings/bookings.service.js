...
let BookingsService = class BookingsService {
    ...
    async resolveAutomationActorId(companyId, userId) {
        if (userId)
            return userId;
    ...
    async enqueueBookingReminder(companyId, userId, bookingId, reasonKey, scheduledFor) {
        const db = this.prisma;
    ...
    async create(companyId, userId, dto) {
        ...
        if (dto.jobId) {
            ...
            if (!job) {
                throw new common_1.BadRequestException('Invalid job for this company');
            }
        }
        ...
        if (dto.assignedUserId) {
            ...
            if (!assignee) {
                throw new common_1.BadRequestException('Invalid assigned user for this company');
            }
        }
    ...
    async ensureBusinessHours(tenantId) {
        const db = this.prisma;
    ...
    async ensureBookingTokens(tenantId) {
        const db = this.prisma;
        ...
        if (!settings.bookingPublicToken) {
            update.bookingPublicToken = crypto_1.default.randomBytes(24).toString('base64url');
        }
        if (!settings.bookingIcsToken) {
            update.bookingIcsToken = crypto_1.default.randomBytes(24).toString('base64url');
        }
    ...
    async getPublicConfig(token) {
        ...
        if (!settings || !settings.bookingPublicEnabled) {
            throw new common_1.BadRequestException('Booking portal not available');
        }
        ...
        const proEnabled = (0, feature_flags_1.isBookingProV1Enabled)();
    ...
    async getAvailableSlots(token, date, serviceId, locationId) {
        const db = this.prisma;
        const settings = await db.tenantSetting.findFirst({ where: { bookingPublicToken: token } });
        if (!settings || !settings.bookingPublicEnabled) {
            throw new common_1.BadRequestException('Booking portal not available');
        ...
        if ((0, feature_flags_1.isBookingProV1Enabled)()) {
        ...
        if ((0, feature_flags_1.isLocationsAdvancedV1Enabled)() && activeLocation) {
        ...
        for (let minute = dayHours.startMinute; minute + durationMinutes <= dayHours.endMinute; minute += slotMinutes) {
            ...
            if (slotStart < minBookAt)
                continue;
            const slotEnd = new Date(slotStart.getTime() + durationMinutes * 60000);
            const overlapping = bookings.filter((booking) => booking.startsAt < slotEnd && booking.endsAt > slotStart).length;
    ...
    async createPublicBooking(token, dto) {
        const db = this.prisma;
        const settings = await db.tenantSetting.findFirst({ where: { bookingPublicToken: token } });
        if (!settings || !settings.bookingPublicEnabled) {
            throw new common_1.BadRequestException('Booking portal not available');
        ...
        if (Number.isNaN(startsAt.getTime()))
            throw new common_1.BadRequestException('Invalid booking time');
        const proEnabled = (0, feature_flags_1.isBookingProV1Enabled)();
    ...
    async getIcsFeed(token) {
        ...
        if (!settings) {
            throw new common_1.BadRequestException('Invalid ICS token');
        }
        const bookings = await db.booking.findMany({
    ...
    async listProServices(companyId, locationId) {
        const db = this.prisma;
    ...
    async createProService(companyId, userId, dto) {
        const db = this.prisma;
    ...
    async computeProSlots(companyId, query) {
        const db = this.prisma;
    ...
    async getProAvailability(companyId, query) {
        return this.computeProSlots(companyId, query);
    }
    async createProBooking(companyId, userId, dto) {
        const db = this.prisma;
    ...
    async getProSettings(companyId) {
        const db = this.prisma;
    ...
    async updateProSettings(companyId, userId, dto) {
        const db = this.prisma;
        if (typeof dto.publicEnabled === 'boolean') {
            await db.tenantSetting.update({
        ...
        if (Array.isArray(dto.questions)) {
            ...
            for (const question of dto.questions) {
                if (!question?.label || !question?.questionKey)
                    continue;
                if (question.id) {
