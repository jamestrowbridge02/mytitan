...
type AutopilotStatus = 'healthy' | 'degraded' | 'attention_needed' | 'down';
  ...
  private async executeSentinels(persist: boolean) {
    ...
    const results: SentinelResult[] = [
      ...
      },
      {
        ...
        label: 'Calendar V2 route',
        status: calendar.ok ? 'healthy' : 'down',
        summary: calendar.ok ? 'Calendar V2 route is reachable.' : 'Calendar V2 route is not reachable.',
        detail: 'Probes the intended /dashboard/calendar route rather than the legacy scheduling route.',
        failedRoute: calendar.ok ? null : '/dashboard/calendar',
        safeFixAction: 'refresh_platform_readiness',
        owner: 'Scheduling',
        impact: 'Operators may land on an obsolete scheduling surface.',
