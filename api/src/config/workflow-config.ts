import { getBusinessConfig } from "../common/business-config";
...
export function getBookingStages(settings?: { businessConfigJson?: unknown } | null) {
  return mergeStages(getBusinessConfig(settings).workflowStages?.bookings, DEFAULT_BOOKING_STAGES);
}

export function getJobStages(settings?: { businessConfigJson?: unknown } | null) {
  return mergeStages(getBusinessConfig(settings).workflowStages?.jobs, DEFAULT_JOB_STAGES);
}

export function getTechnicianStages(settings?: { businessConfigJson?: unknown } | null) {
