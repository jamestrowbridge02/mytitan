...
async function ensureJobsAndBookings(companyId, locationId, serviceItems, demoUserId) {
  ...
  //  seed uses relative timestamps so bookings/jobs stay fresh on each run.
  const now = new Date();
  const today = startOfToday(now);
  // Curated  set (about 6 jobs, 6 bookings) with stable identifiers.
  const jobSeeds = [
    { id: '-job-1', jobRef: '-0001', customerIndex: 0, scenario: 'in_progress' },
    { id: '-job-2', jobRef: '-0002', customerIndex: 1, scenario: 'completed_unpaid' },
    { id: '-job-3', jobRef: '-0003', customerIndex: 2, scenario: 'completed_paid' },
    { id: '-job-4', jobRef: '-0004', customerIndex: 3, scenario: 'awaiting_approval' },
    { id: '-job-5', jobRef: '-0005', customerIndex: 4, scenario: 'unassigned_starting_soon' },
    { id: '-job-6', jobRef: '-0006', customerIndex: 5, scenario: 'open' },
  ];
  const bookingSlots = [
    { id: '-booking-1', customerIndex: 0, start: addMinutes(today, 9 * 60), durationMins: 90, status: 'CONFIRMED', jobRef: '-0001' },
    { id: '-booking-2', customerIndex: 1, start: addMinutes(today, 11 * 60), durationMins: 60, status: 'PLANNED', jobRef: '-0002' },
    { id: '-booking-3', customerIndex: 2, start: addMinutes(today, 13 * 60 + 30), durationMins: 90, status: 'CONFIRMED', jobRef: '-0003' },
    { id: '-booking-4', customerIndex: 3, start: addMinutes(today, 15 * 60), durationMins: 60, status: 'CONFIRMED', jobRef: '-0004' },
    { id: '-booking-5', customerIndex: 4, start: addMinutes(now, -20), durationMins: 75, status: 'PLANNED', jobRef: null },
    { id: '-booking-6', customerIndex: 5, start: addMinutes(now, 45), durationMins: 60, status: 'CONFIRMED', jobRef: '-0005' },
  ];
  let jobsCount = 0;
  let bookingsCount = 0;
  const seededJobs = {};
  ...
  for (const seed of jobSeeds) {
    ...
    const reg = `${100 + seed.customerIndex}`;
    ...
    const job = await prisma.job.upsert({
      ...
      create: {
        ...
        paymentLinkUrl: isCompletedUnpaid ? `https://pay.mytitan./invoice/${seed.jobRef}` : null,
    ...
    await prisma.jobAsset.upsert({
      where: { id: `-before-${job.id}` },
      create: {
        id: `-before-${job.id}`,
        jobId: job.id,
        kind: 'BEFORE',
        url: `https://cdn.mytitan./assets/${job.id}/before.jpg`,
        mime: 'image/jpeg',
      },
      update: {
        url: `https://cdn.mytitan./assets/${job.id}/before.jpg`,
    ...
    await prisma.jobAsset.upsert({
      where: { id: `-after-${job.id}` },
      create: {
        id: `-after-${job.id}`,
        jobId: job.id,
        kind: 'AFTER',
        url: `https://cdn.mytitan./assets/${job.id}/after.jpg`,
        mime: 'image/jpeg',
      },
      update: {
        url: `https://cdn.mytitan./assets/${job.id}/after.jpg`,
    ...
    if (isInProgress) {
      await prisma.jobActivity.upsert({
        where: { id: `-activity-${job.id}-status` },
        create: {
          id: `-activity-${job.id}-status`,
          companyId,
          jobId: job.id,
      ...
      await prisma.jobActivity.upsert({
        where: { id: `-activity-${job.id}-note` },
        create: {
          id: `-activity-${job.id}-note`,
          companyId,
          jobId: job.id,
          actorUserId: demoUserId || null,
          eventType: 'note',
    ...
    if (isCompletedUnpaid) {
      await prisma.jobActivity.upsert({
        where: { id: `-activity-${job.id}-invoice` },
        create: {
          id: `-activity-${job.id}-invoice`,
          companyId,
          jobId: job.id,
          actorUserId: demoUserId || null,
          eventType: 'invoice',
  ...
  if (demoUserId) {
    const commsJob = seededJobs['-0004'] || seededJobs['-0002'] || seededJobs['-0001'];
    if (commsJob) {
      await prisma.notification.upsert({
        where: { id: '-notification-job-1' },
        create: {
  ...
  if (demoUserId) {
    const commsBookingId = bookingSlots[0]?.id || '-booking-1';
    await prisma.notification.upsert({
      where: { id: '-notification-booking-1' },
      create: {
  ...
  // Note: no safe archive field for Job/Booking in schema, so legacy  records remain.
...
async function ensureDemoTradePack(companyId) {
  ...
  if (existing && existing.configJson && existing.configJson.active === false) {
    await prisma.tradePackInstall.update({
      ...
      data: {
        configJson: { active: true, seededBy: 'seed-' },
      },
    });
    return 'reactivated';
  }
