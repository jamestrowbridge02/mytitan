...
- `MYTITAN_FEATURE_DEMO_POLISH_V1=off` (feature flag for demo UX polish + command centre/CRM/booking polish)
- `MYTITAN_FEATURE_MEDIA_SIGNATURE_V1=off` (feature flag for media capture + signature capture/storage)
- `MYTITAN_FEATURE_NOTIFICATIONS_V1=off` (feature flag for in-app notifications + preferences APIs)
...
- `NEXT_PUBLIC_MYTITAN_FEATURE_DEMO_POLISH_V1=off`
- `NEXT_PUBLIC_MYTITAN_FEATURE_MEDIA_SIGNATURE_V1=off`
- `NEXT_PUBLIC_MYTITAN_FEATURE_NOTIFICATIONS_V1=off`

## API Setup and Run

## Notifications send idempotency
- `POST /notifications/send` supports `Idempotency-Key` header.
- Uniqueness is scoped to the tenant/company.
- Retries with the same key return the original metadata record (no duplicate).
- Recommended: generate a UUID per user action/automation trigger.
...
Fix: Dockerfile guard `RUN test -d .next` right after `npm run build`.
Recovery: `docker compose build --no-cache app` (optional `docker builder prune`), then rerun the full chain.

## Feature Gating Policy (API)
Mutations: when a feature is OFF, return 503 with `{ code: 'FEATURE_DISABLED', feature: '<name>' }`.
Reads: only explicitly whitelisted read endpoints may return soft fallbacks (empty lists, zeros). All others return 503.
Auth guards still run first, so unauthenticated requests keep returning 401.
...
## Demo Polish Smoke Script

Run:

```bash
cd /opt/mytitan
./scripts/demo-polish-smoke.sh
```

Checks:
- API + app public health responses
- demo login response when demo flag is on
- auth-protected existence (401/403/400, not 404) for:
  - `GET /notifications`
  - `GET /notifications/preferences`
  - `PATCH /notifications/preferences`
  - `GET /jobs/board-v2`
  - bulk jobs endpoint (`POST /jobs/bulk-v2` or fallback `POST /jobs/bulk`)
...
Use the read-only route inventory generator to refresh audit docs and detect route drift:

```bash
cd /opt/mytitan
./scripts/generate-routes-inventory.sh
```

This regenerates `docs/audit/routes-inventory.md` from NestJS controller decorators and Next.js pages routes.
