...
3. `-polish-smoke.sh` does not read `.env`, it checks shell env only.

##  Smoke Warning Root Cause
Warning observed:
- `WARN:  flag appears off; observed code=200`

Likely cause:
- In `scripts/-polish-smoke.sh`, branch logic uses process env vars:
  - `MYTITAN_FEATURE_PUBLIC_DEMO`
  - `NEXT_PUBLIC_MYTITAN_FEATURE_PUBLIC_DEMO`
- If these are not exported in the shell, script assumes “off” even when container runtime `.env` has  enabled.
- Endpoint result (`200`) proves server flag path is actually enabled.

Safe fix path (reversible):
1. Align -polish smoke with release-smoke behavior by reading `/opt/mytitan/.env` for  flag state.
...
3. Fix `-polish-smoke.sh` env-source mismatch (script-only change).
