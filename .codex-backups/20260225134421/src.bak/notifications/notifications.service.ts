...
import { isNotificationsV1Enabled } from '../common/feature-flags';
...
export class NotificationsService {
  ...
  private async sendViaSmtp(host: string, port: number, user: string, pass: string, from: string, to: string, subject: string, text: string) {
  ...
  private async sendNotificationEmail(companyId: string, to: string, subject: string, text: string) {
  ...
  async notifyJobCompleted(companyId: string, jobId: string) {
    ...
    owners.forEach((o: any) => recipientMap.set(o.id, o.email));
    if (job.assignedUserId && job.assignedUser?.email) recipientMap.set(job.assignedUserId, job.assignedUser.email);
    ...
    for (const [userId, email] of recipientMap.entries()) {
      ...
      await this.sendNotificationEmail(
        companyId,
        email,
        `Job complete: ${job.jobRef || job.id}`,
        textLines.join('\n'),
      );
    }
  }
  ...
  async notifyPaymentReceived(companyId: string, jobId: string) {
    ...
    const recipientMap = new Map<string, string>();
    owners.forEach((o: any) => recipientMap.set(o.id, o.email));
    if (job.createdByUserId && job.createdByUser?.email) recipientMap.set(job.createdByUserId, job.createdByUser.email);
