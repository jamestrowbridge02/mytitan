...
export default function TradeAccountProfilePage() {
  ...
  async function load() {
    ...
    try {
      const [accountPayload, timelinePayload, crmDraft] = await Promise.all([
        apiFetch(`/trade-accounts/${id}`),
        apiFetch(`/trade-accounts/${id}/timeline?page=1&pageSize=25`),
  ...
  const summary = data?.summary || { unpaidCount: 0, unpaidTotalCents: 0, paidTotalCents: 0 };
    ...
    <DashboardShell>
      ...
      <div className="card" style={{ marginBottom: 16 }}>
        ...
        <div style={{ display: 'grid', gap: 10 }}>
          <a className="button" href={`/dashboard/jobs/new?tradeAccountId=${id}`}>Create Job</a>
          <a className="button secondary" href={`/dashboard/bookings?tradeAccountId=${id}`}>Create Booking</a>
