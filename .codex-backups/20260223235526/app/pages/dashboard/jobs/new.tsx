...
export default function NewJobPage() {
  ...
  function validateGuidedStep(stepIndex: number) {
    ...
    if (stepIndex === 0) {
      ...
      if (!String(formData.jobDate || "").trim()) return humanError("Please choose the job start date.");
  ...
  function resumeDraft() {
    ...
    if (!resumeData.jobDate) resumeData.jobDate = todayIso();
    if (!resumeData.completedDate) resumeData.completedDate = todayIso();
  ...
  function renderGuidedStep() {
    ...
    if (step === 0) {
      return sectionCard("Step 1: Job Details", (
        <>
          ...
          <input className="input" type="date" value={formData.jobDate || ""} onChange={(e) => setField("jobDate", e.target.value)} />

          <label>Job completed date (optional)</label>
          <input className="input" type="date" value={formData.completedDate || ""} onChange={(e) => setField("completedDate", e.target.value)} />
    ...
    return sectionCard("Step 6: Pricing + Invoice + WhatsApp + Signatures", (
      <>
        ...
        <input className="input" type="number" value={formData.unitPrice || ""} onChange={(e) => setField("unitPrice", e.target.value)} />

        <label>Service quantity</label>
        <input className="input" type="number" value={formData.quantity || 1} onChange={(e) => setField("quantity", e.target.value)} />
