      ...
      {openedJob && premiumEnabled ? (
        <div className="card" style={{ position: 'fixed', right: 8, top: 80, width: 'min(420px,95vw)', zIndex: 40, maxHeight: '85vh', overflow: 'auto' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <h3 style={{ marginTop: 0 }}>{openedJob.jobRef}</h3>
          ...
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            <a className="button secondary" href={`/dashboard/jobs`}>Quick invoice/pay link</a>
            <a className="button secondary" href={`/dashboard/bookings?jobId=${openedJob.id}`}>Booking link</a>
          </div>
        </div>
      ) : null}
    </DashboardShell>
