    ...
    <DashboardShell>
      <div className="card" style={{ marginBottom: 16 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(180px,1fr))', gap: 10 }}>
          {actions.map((item) => (
            <a key={item.key} className={`button ${item.key === action ? '' : 'secondary'}`} href={`/dashboard/guided?action=${item.key}`}>{item.label}</a>
          ))}
        </div>
      ...
      {action === 'create_job' ? (
        <div className="card">
          ...
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            <a className="button" href="/dashboard/jobs/new?guided=1&resumeTrade=WHEELS">Start Wheels flow</a>
            <a className="button secondary" href="/dashboard/jobs/new">Start standard job</a>
            <a className="button secondary" href="/dashboard">Skip</a>
          </div>
        </div>
      ) : null}
      ...
      {action === 'book_appointment' ? (
        <div className="card">
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            ...
            <a className="button secondary" href="/dashboard/bookings">Open full booking setup</a>
            <a className="button secondary" href="/dashboard">Skip</a>
          </div>
        </div>
      ) : null}
      ...
      {action === 'take_payment' ? (
        <div className="card">
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            <a className="button" href="/dashboard/billing">Open billing</a>
            <a className="button secondary" href="/dashboard/jobs">Open jobs to share pay link</a>
            <a className="button secondary" href="/dashboard">Skip</a>
          </div>
        </div>
      ) : null}
      ...
      {action === 'order_parts' ? (
        <div className="card">
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            ...
            <a className="button secondary" href="/dashboard/inventory">Open inventory dashboard</a>
            <a className="button secondary" href="/dashboard">Skip</a>
          </div>
        </div>
      ) : null}
      ...
      {action === 'message_customer' ? (
        <div className="card">
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            <a className="button" href={whatsappHref()} target="_blank" rel="noreferrer">Open WhatsApp</a>
