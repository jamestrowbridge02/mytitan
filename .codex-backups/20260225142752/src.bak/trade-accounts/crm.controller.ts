...
import { isCrmProV1Enabled } from '../common/feature-flags';
...
export class CrmController {
  ...
  search(@CurrentUser() user: JwtPayload, @Query() query: CrmSearchQueryDto) {
    featureGate({ enabled: isCrmProV1Enabled(), feature: 'CRM_PRO_V1', mode: 'read' });
  ...
  addNote(@CurrentUser() user: JwtPayload, @Param('id') id: string, @Body() dto: AddCrmNoteDto) {
    featureGate({ enabled: isCrmProV1Enabled(), feature: 'CRM_PRO_V1', mode: 'mutation' });
