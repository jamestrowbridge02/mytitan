...
import { isCommandCentrePremiumV1Enabled, isCommandCentreV1Enabled } from '../common/feature-flags';
...
export class CommandCentreController {
  ...
  async summary(@CurrentUser() user: JwtPayload) {
    featureGate({ enabled: isCommandCentreV1Enabled(), feature: 'COMMAND_CENTRE_V1', mode: 'read' });
  ...
  async listViews(@CurrentUser() user: JwtPayload) {
    const fallback = featureGate({
      enabled: isCommandCentrePremiumV1Enabled(),
  ...
  async createView(
    ...
    featureGate({ enabled: isCommandCentrePremiumV1Enabled(), feature: 'COMMAND_CENTRE_PREMIUM_V1', mode: 'mutation' });
    const db = this.prisma as any;
    if (body?.isDefault) {
