...
import { isMarketplaceEnabled, isPublicDemoEnabled, requireMarketplaceEnabled } from "../common/feature-flags";
...
export class PublicController {
  ...
  private async resolveToken(token: string) {
    ...
    const record = await db.publicJobToken.findUnique({
      where: { token },
      include: { job: { include: { media: true, assets: true, pdf: true, company: true } } },
    });
    if (!record) {
