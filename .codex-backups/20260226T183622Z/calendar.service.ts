...
export class CalendarService {
  ...
  async suggestSlots(tenantId: string, bookingId: string, windowDays: number, limit: number) {
    ...
    const exceptionsByTech = new Map<string, Array<{ startsAt: Date; endsAt: Date }>>();
    if (techIds.length) {
      const exceptionRows = await db.techScheduleException.findMany({
        where: {
          companyId: tenantId,
          technicianId: { in: techIds },
          startsAt: { lt: windowEnd },
      ...
      for (const entry of exceptionRows) {
        if (!entry.technicianId) continue;
        const list = exceptionsByTech.get(entry.technicianId) ?? [];
        const start = entry.startsAt instanceof Date ? entry.startsAt : new Date(entry.startsAt);
        const end = entry.endsAt instanceof Date ? entry.endsAt : new Date(entry.endsAt);
        if (Number.isNaN(start.getTime()) || Number.isNaN(end.getTime())) continue;
        list.push({ startsAt: start, endsAt: end });
        exceptionsByTech.set(entry.technicianId, list);
      }
    }
    ...
    if (suggestions.length > 0) {
      ...
      const augmented = new Set<SuggestionReason>([...primary.reasons, 'EARLIEST_AVAILABLE']);
      primary.reasons = this.sortSuggestionReasons(Array.from(augmented));
    }
