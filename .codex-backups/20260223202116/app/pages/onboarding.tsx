...
import { isMarketplaceEnabled, isTradePacksEnabled } from '../lib/feature-flags';
...
export default function OnboardingPage() {
  ...
  useEffect(() => {
    if (!tradePacksEnabled) return;
    const loadTradePacks = async () => {
      try {
        const data = await apiFetch('/trade-packs');
        const list = Array.isArray(data) ? data : [];
        setTradePacks(list);
        const installed = list.find((pack: TradePack) => pack.installed);
        if (installed) {
      ...
      if (step === 2) {
        await apiFetch('/onboarding/step', {
          ...
          body: JSON.stringify({
            step,
            data: {
              packCode: selectedPack || undefined,
            },
          }),
        });
      }
      ...
      <div className="card">
        <div className="wizard-header">
          <div>
            ...
            <p className="muted">Seven quick steps so you can start taking customers today.</p>
          </div>
        ...
        {step === 2 && (
          <>
            <p className="muted">Choose an industry pack to preload services, templates, and defaults. You can skip this and do it later.</p>
            {tradePacksEnabled ? (
              <div className="list">
                {tradePacks.map((pack) => (
                  <div key={pack.code} className="integration-card">
                    <div>
                      ...
                      <p className="muted">What you get: {pack.includes?.join(', ')}</p>
                    </div>
                    <div className="integration-actions">
                      {pack.installed ? <span className="badge">Installed</span> : null}
                      <button
                        className={`toggle ${selectedPack === pack.code ? 'on' : ''}`}
                        type="button"
                        onClick={() => setSelectedPack((prev) => (prev === pack.code ? '' : pack.code))}
                      >
        ...
        {step === 5 && (
          <>
            ...
            <a className="button secondary" href="/dashboard/billing">Open billing setup</a>
          </>
        )}
