...
import { isMarketplaceEnabled, isTradePacksEnabled, requireMarketplaceEnabled } from '../common/feature-flags';
import { Roles } from '../common/roles.decorator';
import { RolesGuard } from '../common/roles.guard';
import { PrismaService } from '../prisma/prisma.service';
import { TenantService } from '../tenant/tenant.service';
import { TradePacksService } from '../trade-packs/trade-packs.service';
import { IntegrationToggleDto, OnboardingStepDto } from './onboarding.dto';
  ...
  async step(@CurrentUser() user: JwtPayload, @Body() dto: OnboardingStepDto) {
    ...
    if (step === 2) {
      ...
      if (packCode && isTradePacksEnabled()) {
        ...
        await this.tradePacks.install(user.companyId, user.sub, packCode);
      }
    }
