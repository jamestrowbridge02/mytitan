...
export default function CalendarPage() {
  ...
  const isSuggestEligible = useCallback((booking: CalendarBlock) => {
    ...
    return (booking.warnings ?? []).some((warning) => warning.code !== 'OVERLAP');
  }, [schedulingEnabled]);
                                        ...
                                        {(suggestState?.suggestions || []).slice(0, 3).map((suggestion) => (
                                          ...
                                          >
                                            <div style={{ fontSize: 11 }}>
                                              ...
                                              {suggestion.reasons?.includes('OVER_CAPACITY') ? (
                                                <span className="muted" style={{ marginLeft: 6 }}>(Over capacity)</span>
                                              ) : null}
                                            </div>
                                            <button
                                              className="button secondary"
