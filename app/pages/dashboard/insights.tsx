...
const HEATMAP_STATUS_STYLES: Record<CapacityStatus, { background: string; color: string }> = {
  OFF: { background: "rgba(148, 163, 184, 0.2)", color: "#475569" },
  UNDER: { background: "rgba(16, 185, 129, 0.2)", color: "#047857" },
  OK: { background: "rgba(59, 130, 246, 0.15)", color: "#1d4ed8" },
  OVER: { background: "rgba(248, 113, 113, 0.25)", color: "#991b1b" },
};
...
export default function InsightsPage() {
  ...
  const [capacityData, setCapacityData] = useState<CapacityHeatmapResponse | null>(null);
  const [capacityLoading, setCapacityLoading] = useState(false);
  const [capacityError, setCapacityError] = useState("");
  const [capacityRequestId, setCapacityRequestId] = useState<string | undefined>(undefined);
  ...
  async function loadCapacity() {
    ...
    setCapacityError("");
    setCapacityRequestId(undefined);
    try {
      const response = (await apiFetch("/calendar/capacity")) as CapacityHeatmapResponse;
      setCapacityData(response || null);
    } catch (err: any) {
      setCapacityError(err?.message || "Failed to load capacity heatmap");
      setCapacityRequestId(err instanceof ApiError ? err.requestId : undefined);
    } finally {
      setCapacityLoading(false);
    }
  }

  const handleHeatmapNavigation = (day: string, techId: string) => {
    router.push({
  ...
  return (
    <DashboardShell>
      ...
      {schedulingEnabled ? (
        <div className="card" style={{ marginTop: 16 }}>
          <h2 style={{ marginTop: 0 }}>Capacity Heatmap (next 7 days)</h2>
          <p className="muted">See how each technician is tracking against their working hours.</p>
          {capacityLoading ? (
            <LoadingState title="Loading capacity" description="Aggregating schedule data." />
          ) : null}
          {!capacityLoading && capacityError ? (
            <ErrorState
              ...
              primaryAction={{ label: "Try again", onClick: loadCapacity }}
            />
          ) : null}
          {!capacityLoading && !capacityError && capacityData && capacityData.technicians.length > 0 ? (
            <div style={{ marginTop: 12, overflowX: "auto" }}>
              <div
                style={{
                  minWidth: Math.max(320, capacityData.days.length * 140),
                  display: "grid",
                  gridTemplateColumns: `200px repeat(${capacityData.days.length}, minmax(120px, 1fr))`,
                  gap: 6,
                }}
              >
                <div style={{ fontWeight: 600, padding: 4 }}>Technician</div>
                {capacityData.days.map((dayKey) => (
                  <div key={`header-${dayKey}`} style={{ padding: 4, textTransform: "uppercase" }}>
                ...
                {capacityData.technicians.map((tech) => (
                  <Fragment key={tech.id}>
                    <div style={{ padding: "6px 4px", fontWeight: 600 }}>{tech.name || "Technician"}</div>
                    {capacityData.days.map((dayKey) => {
                      const cell = capacityData.capacity[tech.id]?.[dayKey];
                      const status = cell?.status ?? "OFF";
                      const style = HEATMAP_STATUS_STYLES[status];
                      const utilizationLabel = cell ? `${cell.utilizationPct}%` : "0%";
                      const booked = formatMinutesToHours(cell?.bookedMinutes ?? 0);
                      const available = formatMinutesToHours(cell?.availableMinutes ?? 0);
                      return (
                        <button
                          key={`${tech.id}-${dayKey}`}
                          type="button"
                          onClick={() => handleHeatmapNavigation(dayKey, tech.id)}
                          style={{
                            borderRadius: 8,
                        ...
                        >
                          <span style={{ fontWeight: 600 }}>{utilizationLabel}</span>
          ...
          {!capacityLoading && !capacityError && capacityData && capacityData.technicians.length === 0 ? (
            <EmptyState title="No technicians to show" description="Technicians will appear once they have schedule data." />
          ) : null}
        </div>
      ) : null}
    </DashboardShell>
